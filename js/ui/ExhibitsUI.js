...
export function buildExhibit(size) {
    ...
    // 🔥 NEW: Prompt for Habitat Type
    const habitatChoice = prompt(
        `What type of habitat is this?\n1. Terrestrial (Land)\n2. Aquatic (Water)\n3. Terrarium (Reptiles/Glass)\n\nEnter a number (1-3):`, 
        "1"
    );
    
    let habitatType = 'terrestrial'; // Default
    if (habitatChoice === '2') habitatType = 'aquatic';
    else if (habitatChoice === '3') habitatType = 'terrarium';
    ...
    state.exhibits[newId] = {
        ...
        type: habitatType, // 🔥 Now uses the player's choice!
        zone: finalZone, 
        animals: [],
        upgrades: [],
...
export function openTransferModal(currentExhibitId, animalIdentifier) {
    ...
    for (const id in state.exhibits) {
        ...
        // TYPE CHECK: Only show exhibits matching the animal's required type
...
function getUniqueZones() {
    const zones = new Set();
    Object.values(state.exhibits).forEach(ex => {
        if (ex.zone) zones.add(ex.zone);
    });
...
export function renameExhibitZone(exhibitId) {
    ...
    if (newName && newName.trim() !== '') {
        exhibit.zone = newName.trim();
        renderExhibits();
    }
}

window.setZoneFilter = (zoneId) => {
