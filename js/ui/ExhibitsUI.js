...
import { EXHIBIT_SIZES, HABITAT_TYPES, getLifeStage } from '../engine/constants.js';
...
export function renderExhibits() {
    ...
    // 2. Build New Exhibit Section (NESTED LOOP FOR ALL COMBINATIONS)
    html += `
        <div class="status-panel">
            <h3>🏗️ Build New Exhibit</h3>
            <p style="color: #9ca3af; margin-bottom: 15px;">Choose a size and habitat type.</p>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 15px;">
    `;

    for (const sizeKey in EXHIBIT_SIZES) {
        const sizeData = EXHIBIT_SIZES[sizeKey];
        for (const typeKey in HABITAT_TYPES) {
            const typeData = HABITAT_TYPES[typeKey];
            
            const finalCost = typeData.baseCost * sizeData.costMultiplier;
            ...
            html += `
                <div style="background: #0f172a; border: 1px solid #334155; border-radius: 10px; padding: 15px; display: flex; flex-direction: column;">
                    ...
                    <h4 style="margin: 0 0 4px; color: #e5e7eb; text-align: center; font-size: 1rem;">${sizeData.name} ${typeData.name}</h4>
    ...
    // 3. Current Exhibits Section (Filtered by Zone)
    ...
    if (filteredExhibitIds.length === 0) {
        ...
        filteredExhibitIds.forEach(id => {
            ...
            const sizeData = EXHIBIT_SIZES[exhibit.size] || EXHIBIT_SIZES.small;
            const typeData = HABITAT_TYPES[exhibit.type] || HABITAT_TYPES.terrestrial;
            
            const finalUpkeep = typeData.baseUpkeep * sizeData.upkeepMultiplier;
            ...
            const typeBadge = `<span style="background: ${typeData.id === 'terrarium' ? '#f59e0b' : typeData.id === 'aquatic' ? '#3b82f6' : '#22c55e'}; color: #000; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-left: 8px;">${typeData.icon} ${typeData.name}</span>`;
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; flex-wrap: wrap; gap: 10px; margin-bottom: 10px;">
                        <div>
                            ...
                            <div style="color: #9ca3af; font-size: 0.85rem; margin-top: 4px;">
                                ${sizeData.name} • ${exhibit.animals.length}/${sizeData.maxAnimals} animals • $${finalUpkeep}/day upkeep
                            </div>
                        </div>
...
function renderAnimalCard(animal, exhibitId) {
    ...
    return `
        <div style="background: #1e293b; border: 1px solid #334155; border-radius: 8px; padding: 10px; cursor: pointer;" onclick="window.showAnimalDetails('${exhibitId}', '${animalIdentifier}')">
            ...
            ${animal.isPregnant ? `<div style="background: rgba(236, 72, 153, 0.15); border: 1px solid #ec4899; border-radius: 6px; padding: 6px; margin-top: 6px;"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;"><span style="font-size: 0.75rem; color: #f9a8d4; font-weight: 700;"> Pregnant</span><span style="font-size: 0.75rem; color: #ec4899; font-weight: 700;">${animal.daysUntilBirth}d left</span></div><div style="height: 4px; background: #1e293b; border-radius: 2px; overflow: hidden;"><div style="height: 100%; width: ${getPregnancyProgress(animal)}%; background: linear-gradient(90deg, #ec4899, #f472b6);"></div></div></div>` : ''}
...
export function buildExhibit(size, type) {
    ...
    const typeData = HABITAT_TYPES[type];
    if (!sizeData || !typeData) return;
    
    const finalCost = typeData.baseCost * sizeData.costMultiplier;
    if (state.money < finalCost) {
        alert(`Not enough money! Need $${finalCost}`);
        return;
    }
    
    const name = prompt(`Name your new ${sizeData.name} ${typeData.name}:`, `${typeData.name} ${Object.keys(state.exhibits).length + 1}`);
    if (!name) return;

    const zoneName = prompt(`What Zone does this belong to? (e.g., "Savanna", "Reptile House")`, "General");
    ...
    eventBus.emit('EXHIBIT_BUILD_STARTED', { name: name, size: size, type: type, cost: finalCost, days: sizeData.buildDays });
    renderExhibits();
...
export function showAnimalDetails(exhibitId, animalIdentifier) {
    ...
    alertBox.innerHTML = `
        <div style="background: #1e293b; padding: 30px; border-radius: 12px; max-width: 500px; width: 90%; border: 2px solid #334155; max-height: 90vh; overflow-y: auto;">
            ...
            ${animal.isPregnant ? `<div style="background: rgba(236, 72, 153, 0.1); border: 1px solid #ec4899; padding: 10px; border-radius: 6px; margin-bottom: 15px;"><div style="color: #ec4899; font-weight: 700;">🤰 Pregnant</div><div style="color: #e5e7eb; font-size: 0.9rem;">Baby due in ${animal.daysUntilBirth} day${animal.daysUntilBirth !== 1 ? 's' : ''}</div><div style="height: 6px; background: #1e293b; border-radius: 3px; overflow: hidden; margin-top: 6px;"><div style="height: 100%; width: ${getPregnancyProgress(animal)}%; background: linear-gradient(90deg, #ec4899, #f472b6);"></div></div></div>` : ''}
            ${parentInfo}
...
export function openTransferModal(currentExhibitId, animalIdentifier) {
    ...
    for (const id in state.exhibits) {
        ...
        const sizeData = EXHIBIT_SIZES[exhibit.size];
...
export function showBabyNamingModal(babyInfo) {
    ...
    modal.innerHTML = `<div style="background: linear-gradient(135deg, #1e293b, #334155); padding: 40px; border-radius: 16px; max-width: 500px; width: 90%; border: 3px solid #ec4899; text-align: center;"><div style="font-size: 4rem; margin-bottom: 10px;">🍼</div><h2 style="margin: 0 0 10px; color: #f9a8d4; font-size: 1.8rem;">A New ${babyInfo.species} is Born!</h2><p style="color: #e5e7eb; margin-bottom: 20px;"><strong>${babyInfo.motherName}</strong> has given birth to a ${babyInfo.gender} baby!<br><span style="color: #9ca3af; font-size: 0.9rem;">Father: ${babyInfo.fatherName}</span></p><label style="display: block; color: #e5e7eb; font-weight: 700; margin-bottom: 8px; text-align: left;">Name your new baby:</label><input type="text" id="babyNameInput" value="${defaultName}" maxlength="20" style="width: 100%; padding: 12px; font-size: 1.1rem; background: #0f172a; color: #e5e7eb; border: 2px solid #ec4899; border-radius: 8px; text-align: center; margin-bottom: 10px;" placeholder="Enter a name..."><button onclick="window.randomizeBabyName('${babyInfo.species}', '${babyInfo.gender}')" style="padding: 6px 12px; background: #334155; color: #e5e7eb; border: none; border-radius: 6px; cursor: pointer; font-size: 0.85rem; margin-bottom: 20px;"> Randomize Name</button><div style="display: flex; gap: 10px;"><button onclick="window.confirmBabyName('${babyInfo.babyUid}')" style="flex: 1; padding: 12px; background: #22c55e; color: #000; border: none; border-radius: 8px; font-weight: 700; cursor: pointer; font-size: 1rem;">✅ Confirm Name</button></div></div>`;
    document.body.appendChild(modal);
    setTimeout(() => { const input = document.getElementById('babyNameInput'); if (input) { input.focus(); input.select(); } }, 100);
}
...
export function buyUpgrade(exhibitId, upgradeId) {
    ...
    if (!upgradeData) { alert("Upgrade not found!"); return; }
    const check = canInstallUpgrade(exhibit, upgradeId);
    if (!check.allowed) { alert(`Cannot install: ${check.reason}`); return; }
    if (state.money < upgradeData.cost) { alert(`Not enough money! Need $${upgradeData.cost}`); return; }
    if (!confirm(`Install ${upgradeData.icon} ${upgradeData.name} for $${upgradeData.cost}?`)) return;
    state.money -= upgradeData.cost;
    if (!exhibit.upgrades) exhibit.upgrades = [];
    exhibit.upgrades.push(upgradeId);
    eventBus.emit('UPGRADE_INSTALLED', { exhibitName: exhibit.name, upgradeName: upgradeData.name, cost: upgradeData.cost });
    renderExhibits();
}
...
export function removeUpgrade(exhibitId, upgradeId) {
    ...
    if (!confirm(`Remove ${upgradeData.name}?\nYou'll receive a $${refund} refund (30% of cost).`)) return;
    exhibit.upgrades = exhibit.upgrades.filter(id => id !== upgradeId);
    state.money += refund;
    eventBus.emit('UPGRADE_REMOVED', { exhibitName: exhibit.name, upgradeName: upgradeData.name, refund: refund });
    renderExhibits();
}
...
function generateBabyName(species, gender) {
    ...
    return `${species} ${names[Math.floor(Math.random() * names.length)]}`;
}
window.randomizeBabyName = (species, gender) => { const input = document.getElementById('babyNameInput'); if (input) input.value = generateBabyName(species, gender); };
window.confirmBabyName = (babyUid) => {
    ...
    if (renameBaby(babyUid, newName)) {
        const modal = document.getElementById('babyNamingModal'); if (modal) modal.remove();
        eventBus.emit('BABY_NAMED', { babyUid, newName }); renderExhibits();
    }
};
...
window.attemptBreeding = (exhibitId) => { if (attemptBreeding(exhibitId)) renderExhibits(); };
